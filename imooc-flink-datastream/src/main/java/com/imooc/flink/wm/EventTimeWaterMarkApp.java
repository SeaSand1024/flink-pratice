...
//                System.out.println("-----reduce invoked----" + value1.f0 + "==>" + (value1.f1 + value2.f1));
        ...
        List<HttpHost> httpHosts =
            Lists.newArrayList(new HttpHost("127.0.0.1", 9200, "http"), new HttpHost("127.0.0.1", 9201, "http"));
        ...
        /**
         * WM其实就是延迟触发的一种机制 WM = 数据所携带的时间(窗口中最大的时间) - 延迟执行的时间 WM >= 上一个窗口的结束边界 就会触发窗口的执行 6999 - 2000 = 4999 >= 上一个窗口的结束边界
         */
    }
    ...
    private static ElasticsearchSink.Builder<String> getStringBuilder(List<HttpHost> httpHosts) {
        ElasticsearchSink.Builder<String> esSinkBuilder =
            new ElasticsearchSink.Builder<>(httpHosts, new ElasticsearchSinkFunction<String>() {
                public IndexRequest createIndexRequest(String element) {
                    ...
                    return Requests.indexRequest().index("event_time_water_mark_app")
        ...
        esSinkBuilder.setRestClientFactory(restClientBuilder -> {
            ...
            credentialsProvider.setCredentials(AuthScope.ANY, new UsernamePasswordCredentials("elastic", "test123"));
